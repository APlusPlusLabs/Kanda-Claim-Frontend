...
export default function SubmitAssessment({ params }: Props) {
  ...
  const addPart = () => {
    ...
    // togglePartSelection(newPart.id)
    calculateTotalCost()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        router.push('/dashboard/insurer/assessments')
      }
    } catch (error: any) {
  ...
  if (!assessment) {
    return (
      <div className="text-center py-8">
        ...
        <Button onClick={() => router.push("/dashboard/insurer/assessments")} className="mt-4">
          Back to Assessments
        </Button>
      </div>
    )
    ...
    >
      <div className="space-y-6">
        <Breadcrumb>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink href={`/dashboard/insurer/assessments/${id}`}>{assessment.id}</BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="labor-cost"
                  type="number"
                  placeholder="Enter labor cost" min="0"
                  value={laborCost}
          ...
          <div className="flex justify-end space-x-2">
            <Button variant="outline" type="button" asChild>
              <Link href={`/dashboard/insurer/assessments/${id}`}>Cancel</Link>
            </Button>
