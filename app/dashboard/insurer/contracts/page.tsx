...
export default function FlexibleContractsPage() {
  ...
  const fetchAvailableItems = async (contractableType) => {
  ...
  const handleContractAction = async (contractId, action, data = {}) => {
    ...
    >
      <div className="space-y-6">
        ...
        <ContractDialog
          ...
          contractDraftsByType={contractDraftsByType}
          contractTypeLabels={contractTypeLabels}
          setSelectedContractable={setSelectedContractable}
        />
        ...
        <SendEmailDialog
          ...
          onOpenChange={setSendEmailDialog}
          contract={selectedContractForSigning}
          emailForm={emailForm}
...
function ContractDialog({
  ...
  availableItems,
  contractDraftsByType,
  contractTypeLabels,
  setSelectedContractable,
}) {
  return (
            ...
            {form.watch("contract_type") && (
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      Select {contractTypeLabels[form.watch("contract_type")]} Item
                    </FormLabel>
                    <Select
                      onValueChange={(value) => {
                        field.onChange(value)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Please sign below to approve contract {contract?.code}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="border rounded-lg p-4 bg-gray-50">
            ...
            <p className="text-sm">Value: {formatCurrency(contract?.contract_value)}</p>
          </div>
          <div>
          ...
          <Button
            onClick={() => {
              if (signatureData && contract) {
                handleSignContract(contract.id, signatureData)
              }
            }}
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Send contract {contract?.code} via email
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div>
