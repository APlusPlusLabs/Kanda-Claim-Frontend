...
const authCodeformSchema = z.object({
  authCode: z.string().min(6, "Activation code must be 6 characters"),
  email: z.string().email("Please enter a valid email address"),
})

export default function CheckActivationCodePage() {
  ...
  async function resendCode() {
    if (!authCodeForm.getValues().email) {
      toast({
        variant: "destructive",
    ...
    setIsResending(true)
    try {
      await apiRequest(`${API_URL}users/send-auth-code`, "POST", {
        email: authCodeForm.getValues().email
      })
