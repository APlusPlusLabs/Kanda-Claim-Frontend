...
const Link = React.forwardRef<HTMLAnchorElement, LinkProps>(
  ({ className, variant, size, asChild = false, href, nextLinkProps, ...props }, ref) => {
    const Comp = asChild ? Slot : "a"

    if (href.startsWith("/") || href.startsWith("#")) {
      return (
        <NextLink href={href} {...nextLinkProps} legacyBehavior passHref>
          <Comp className={cn(linkVariants({ variant, size, className }))} ref={ref} {...props} />
        </NextLink>
      )
    }

    return <Comp className={cn(linkVariants({ variant, size, className }))} ref={ref} href={href} {...props} />
  },
)
